...
\begin{document}
    ...
    \begin{table}[h!]
        ...
        (chi tiết ở phụ lục ). Ta có thể sử dụng số bước để so sánh tính hiệu quả của các phương pháp}
    ...
    \begin{table}[h!]
        ...
        Vì vậy ta có thể sử dụng số bước thời gian để so sánh độ hiệu quả của các phương pháp. Để giảm ảnh hưởng của tính ngẫu nhiên, ta cũng lặp lại với cấu hình số bước thời gian nhiều nhất và phương sai sẽ được đề cập ở phần phụ lục .}
        \begin{center}
            \resizebox{\columnwidth}{!}{
    ...
    \begin{bd} (Kết nối moment có điều kiện) Giả sử $q(\boldsymbol{x})$ là một hàm mật độ xác suất với trung bình $\boldsymbol{\mu}_q$ và ma trận hiệp phương sai $\boldsymbol{\Sigma}_q$ và $p(\boldsymbol{x})=\mathcal{N}(\boldsymbol{x} \vert \boldsymbol{\mu}, \mathrm{diag}(\boldsymbol{\sigma}^2))$ là một phân phối Gaussian. Khi đó:
    ...
    Ta khai triển $D_{\mathrm{KL}} \big( \mathcal{N} (\boldsymbol{x} \vert \boldsymbol{\mu}_q, \boldsymbol{\Sigma}_q) \Vert p$ ntheo định nghĩa của độ phân kỳ Kullback-Leibler:
